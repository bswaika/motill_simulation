...
class FailureOrchestrator(Orchestrator):
    ...
    def __init__(self, client: Client, scene: PointCloud, planner: PathPlanner, standalone: bool=True, keep_color: bool=False, p: float=FAILURE_PROBABILITY, delay: float=0) -> None:
        super().__init__()
        self._client = client
        self._scene = scene
        self._delay = delay
    ...
    def _add_extra_drones(self):
        ...
        while i < num_extra_drones:
            ...
            self._client.simAddVehicle(name, deepcopy(curr_positions[station]), self._standby_color)
